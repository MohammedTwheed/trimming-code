...
load('..\training-data\QD.mat');
load('..\training-data\Pow.mat');
...
[finalMSE_H,  bestTrainedNet_H] ...
    = optimizeNNForTrimmingPumpImpeller2([Q';D' ],H');

% [optimalHyperParamsP, finalMSEP, randomSeedP, bestTrainedNetP] = ...
%         optimizeNNForTrimmingPumpImpeller(QD', P', 7156);
...
hold off;
  ...
  filename = sprintf('dr-farid-edit_2_%d-%d-%d-%d.png', optimalHyperParams(1),...
...
function [ finalMSE, bestTrainedNet] = optimizeNNForTrimmingPumpImpeller2(x, t, userSeed)
    % optimizeNNForTrimmingPumpImpeller: Optimizes neural network
    % hyperparameters for pump impeller trimming. Inputs:
    %   x - Input data (Q flowrate, H head) for the neural network. t -
    %   Target data (D diameter or power but not both at same time) for the
