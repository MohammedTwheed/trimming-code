...
for dIdx = 1:length(distinctDiameters)
    ...
    % Initialize bounds
    lower_bounds=[2,      2,    150 , 1, 1];
    upper_bounds=[17,  50,    300, 2, 1];
    ...
    for i = 1:20
        ...
        [optimalHyperParamsH, finalMSEH, randomSeedH, bestTrainedNetH, error] = ...
            optimizeNNForTrimmingPumpImpeller([QH_temp(1,:); D_temp], QH_temp(2,:), userSeed+i,lower_bounds,upper_bounds,removedQH, removedD,QH_beps, D_beps);

        % Store result for this iteration
        result(i, :) = [i, optimalHyperParamsH, finalMSEH, randomSeedH, error(1), error(2), error(3),error(4),error(5),error(6)];
        ...
        filename = sprintf('../loop_06/01/nn_diameter-%d_iteration_%d_%d-%d-%d-%d-%d_mseDia-%d_test-%d_comb-%d.png', diameterToRemove, i, ...
            optimalHyperParamsH(1), optimalHyperParamsH(2), optimalHyperParamsH(3), ...
            optimalHyperParamsH(4), optimalHyperParamsH(5),mseDiameter,error(3),error(6));
        saveas(gcf, filename);

        % Specify the filename for saving the network
        filename = sprintf('../loop_06/01/nn_diameter-%d_iteration_%d_%d-%d-%d-%d-%d_mseDia-%d_test-%d_comb-%d.mat', diameterToRemove, i, ...
            optimalHyperParamsH(1), optimalHyperParamsH(2), optimalHyperParamsH(3), ...
            optimalHyperParamsH(4), optimalHyperParamsH(5), mseDiameter,error(3),error(6));
...
writematrix([["Iteration", "Hidden Layer 1 Size", "Hidden Layer 2 Size", "Max Epochs", ...
    ...
    "Random Seed", "Training Error", "Validation Error", "Test Error",'mseDiameter','mseQH_beps','combinedMSE']; result], './01/results_loop.csv');
...
function [optimalHyperParams, finalMSE, randomSeed, bestTrainedNet,nnPerfVect] = optimizeNNForTrimmingPumpImpeller(x, t, userSeed,lowerBounds,upperBounds,removedQH, removedD,QH_beps, D_beps)
...
% returned by evaluateHyperparameters.
    function [normalizedCombinedMSE] = evaluateHyperparameters(hyperParams, x, t, randomSeed)
        ...
        net = feedforwardnet([2,hiddenLayer1Size,hiddenLayer2Size,1],...
        ...
        combinedMSEVEC = [0.1*avgMSEs , 0.2*mseQH_beps , 0.5*mseDiameter , 0.2*testPerformance];
        combinedMSE =0.1*avgMSEs + 0.2*mseQH_beps + 0.5*mseDiameter + 0.2*testPerformance;

        normalizedCombinedMSE = (combinedMSE - min(combinedMSEVEC)) / (max(combinedMSEVEC) - min(combinedMSEVEC));


        % Check if the current MSE is the best MSE so far and update the
        % global variable if necessary.
        if isempty(bestTrainedNet) || normalizedCombinedMSE < perform(bestTrainedNet, ...
                t, bestTrainedNet(x))
            bestTrainedNet = trainedNet;
            nnPerfVect= [trainPerformance,valPerformance,testPerformance,mseDiameter,mseQH_beps,combinedMSE];
        end
    end
