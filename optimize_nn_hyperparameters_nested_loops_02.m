...
% optimize_nn_hyperparameters_nested_loops_02.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MAIN
...
% Find all distinct diameters in QD
distinctDiameters = unique(QD(2, :));
...
resultsStruct = struct('Timestamp', {}, 'DiameterRemoved', {}, 'Iteration', {}, 'HiddenLayersSizes', {}, ...
    ...
    'ValidationError', {}, 'TestError', {}, 'mseP_beps', {}, 'TrainedNet', {}, 'FigureFileName', {});

for dIdx = 1:length(distinctDiameters)
    % Current diameter to remove
    diameterToRemove = distinctDiameters(dIdx);
    
    % Find indices of the current diameter in QD
    ...
    % Initialize bounds
    lower_bounds = [2,   9,   12, 190, 1, 1];
    upper_bounds = [17, 62,   20, 300, 1, 1];
    
    % Track the previous combined MSE to determine the improvement
    prevCombinedMSE = inf;

    for i = 1:30
        [optimalHyperParamsP, finalMSEP, randomSeedP, bestTrainedNetP, error] = ...
            optimizeNNForTrimmingPumpImpeller([QD_temp(1, :); QD_temp(2, :)], P_temp, userSeed + i, lower_bounds, upper_bounds);

        % Calculate MSE for the removed diameter
        predictedP = bestTrainedNetP([removedQD(1, :); removedQD(2, :)])';
        mseDiameter = mean((removedP' - predictedP).^2 / sum(removedP));

        predictedP_beps = bestTrainedNetP([QD_beps(1, :); QD_beps(2, :)])';
        mseP_beps = mean((P_beps' - predictedP_beps).^2 / sum(P_beps));

        fprintf('Diameter %d, Iteration %d, MSE_Dia: %.6f, MSE_beps: %.6f\n', diameterToRemove, i, mseDiameter, mseP_beps);
        ...
        scatter(removedQD(1, :)', removedP', 'r', 'filled', 'DisplayName', sprintf('Removed Diameter: %dmm', diameterToRemove));
        ...
        for diameterIndex = 1:length(desiredDiameters)
            ...
            filteredP = bestTrainedNetP([Qt; Dt'])';

            % Define legend entry for each diameter
            legendLabel = strcat('Diameter: ', num2str(desiredDiameter), 'mm');

            % Plot Q vs P with appropriate label and legend entry
            plot(Qt, filteredP, 'DisplayName', legendLabel);
            
            % Add a callout marker at the end of the curve
            text(Qt(end), filteredP(end), sprintf('%dmm', desiredDiameter), 'FontSize', 8, 'Color', 'black', 'BackgroundColor', 'white');
        end

        % Plot the remaining original data points
        scatter(QD_temp(1,:)', P_temp', 'b', 'filled', 'DisplayName', 'Reference Points');
        
        xlabel('Q (m^3/h)');
        ylabel('P (kW)');
        title(['(Q, P) slices with Diameters, Removed Diameter: ' num2str(diameterToRemove) 'mm']);
        legend;  
        hold off;

        % Save the plot with a descriptive filename
        filename = sprintf('./optimize_nn_hyperparameters_nested_loops_02/nn_diameter-%d_iteration_%d_mseDia-%.6f_test-%.6f_mseP_beps-%.6f.png', diameterToRemove, i, mseDiameter, error(3), mseP_beps);
        saveas(gcf, filename);

        % Determine the number of hidden layers
        numHiddenLayers = length(optimalHyperParamsP) - 3;

        % Extract hyperparameters
        hiddenLayersSizes = optimalHyperParamsP(1:numHiddenLayers);
        maxEpochs = optimalHyperParamsP(end - 2);
        trainingFunctionIdx = optimalHyperParamsP(end - 1);
        activationFunctionIdx = optimalHyperParamsP(end);
        ...
        newEntry = struct(...
            ...
            'FinalMSE', finalMSEP, ...
            'RandomSeed', randomSeedP, ...
            'TrainingError', error(1), ...
            'ValidationError', error(2), ...
            'TestError', error(3), ...
            'mseP_beps', mseP_beps, ...
            'TrainedNet', bestTrainedNetP, ...
            'FigureFileName', filename);

        % Specify the filename for saving the network
        filename = sprintf('./optimize_nn_hyperparameters_nested_loops_02/nn_diameter-%d_iteration_%d_mseDia-%.6f_test-%.6f_mseP_beps-%.6f.mat', diameterToRemove, i, mseDiameter, error(3), mseP_beps);

        % Save the network to the .mat file
        save(filename, 'bestTrainedNetP');
        ...
        if (mseDiameter < mseThreshold) && (error(3) < 0.0199) && (mseP_beps < mseThreshold)
